...
調べてみると、やり方が大きく 2 つありました。

- **方式A**: API 側に「このロールなら呼んでいい」と書いておく（リソースポリシー方式）
- **方式B**: 呼び出し元がいったん API 側アカウントのロールを借りてから呼ぶ（AssumeRole 方式）

どちらも IAM 認証（SigV4 署名）を使う方法です。せっかくなので両方とも AWS SAM で組んで、実際にデプロイして動かしてみました。この記事はその記録です。
...
:::message
この記事は **IAM 認証（SigV4）を使う前提**の話です。Lambda オーソライザーや JWT オーソライザーで独自トークンを検証する構成なら、そもそもアカウントの壁を IAM で越えないので今回の話は当てはまりません。その代わり鍵やトークンの管理が自前になります。
:::
...
意地悪な仕様に見えますが、**2 つのアカウントの管理者が独立に同意する必要があるから**と考えると納得できます。

B の管理者が勝手に「A の誰でもどうぞ」と決めても、A の管理者が許可しなければ A のリソースは動きません。逆も同じです。つまりこの AND は、**片方のアカウントが単独でアカウントの壁を越えられないようにするための保証**になっているわけですね。同一アカウント内なら管理者は 1 人なので、どちらかに書けば意思表示として十分、という設計だと理解しています。
...
### なぜもう 1 つの方式が要るのか: HTTP API はリソースポリシーが使えない
...
というわけで、HTTP API で IAM 認証を使う場合はこの方式になります。
...
**「壁を越える部分を `execute-api` から `sts:AssumeRole` に付け替えている」** と捉えると、一気にわかりやすくなりました。壁を越える箇所には当然「両側の許可が必要」が適用されるので、AssumeRole には A 側の権限と B 側の信頼ポリシーの両方が必要です。
...
実装してみた印象としては、方式A は構成要素が少なくて素直でした。B 側のログに A のロールがそのまま出るので、動かしていて状況がわかりやすいのも良かったです。

一方の方式B は、キャッシュ処理を書く分だけコードは増えましたが、A 側に書く権限が `sts:AssumeRole` 1 つで済むのは扱いやすいと感じました。B 側の API が増えても A 側を触らなくていいので、対象が増えていく構成なら効いてきそうです。
...
2 つの方式を実際に組んで動かしてみて、わかったことを並べておきます。

- クロスアカウントでは **呼ぶ側と呼ばれる側の両方**で明示的に許可が要る。同一アカウントのつもりで片方だけ書くと通らない
- **HTTP API はリソースポリシー非対応**なので、IAM 認証でクロスアカウントを実現するなら AssumeRole 方式になる
- API に届く `caller.accountId` が、方式A では **呼び出し元アカウント**、方式B では **API 提供側アカウント**になる。この違いがそのまま追跡のしやすさの差になる
- 方式A でも `userArn` は `assumed-role` 形式だった。Lambda はもともと実行ロールを借りて動いている
